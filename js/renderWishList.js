...
const openWishlist = () => {
  const wishlistContainerHTML = `
      <div class="wishlist" id="wishlist" data-active="active">
    <div class="wishlist-container">
      <div class="wishlist-content">
        <h1 class="wishlist-title">MY WISHLIST</h1>
        <h6 class="wishlist-suptitle">${countWishlistButton()} ITEM${
    countWishlistButton() > 1 ? 'S' : ''
  }</h6>
  ...
  header.insertAdjacentHTML('afterend', wishlistContainerHTML)
  const wishlistContainer = document.querySelector('.wishlist-items')

  product.forEach(info => {
    if (!info.liked) return
    const wishlistProductHTML = `
    <div class="wishlist-item" data-id="${info.id}">
      <div class="wishlist-item-content">
        <div class="wishlist-card">
          <div class="wishlist-card-assets">
            <span href="#" class="product-card-assets__img">
              <img src="${info.imgSrc[5]}" alt="">
            </span>
            <div class="wishlist-card-assets__icon wish-button">
            ...
            <span href='#' class="wishlist-card__price">${info.price}</span>
          </div>
          <div class="wishlist-card-details">
            <h6 class="wishlist-card-details__title">
              ${info.title}
            </h6>
          </div>
        </div>
    ...
    `
    wishlistContainer.insertAdjacentHTML('afterbegin', wishlistProductHTML)
  })
}
